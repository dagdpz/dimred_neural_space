...
class dPCA(BaseEstimator):
        ...
        if save_memory:
            ...
            for key, phi in pcombs.items():
                if len(key) == 1:
                    ...
                    pre_mean[key] = mmean(
                        pre_mean[key[:-1]], np.array([list(phi)[-1]]) + 1, expand=True
                    )
            ...
            for key, phi in pcombs.items():
                ...
                if len(key) > 1:
                    ...
                    Xmargs[key] = ne.evaluate(
                        "X - " + " - ".join(subsets), local_dict=local_dict
                    )
                else:
                    Xmargs[key] = X
    ...
    def _optimize_regularization(self, X, trialX, center=True, lams="auto"):
        ...
        # center data
        if center:
            X = X - np.mean(X.reshape((X.shape[0], -1)), 1).reshape(
                (X.shape[0],) + len(self.labels) * (1,)
            )
        ...
        totalscore = np.mean(
            np.sum(np.dstack([scores[key] for key in list(scores.keys())]), -1), 0
        )
    ...
    def crossval_score(self, lams, X, trialX, mean=True):
        ...
        scores = (
            np.zeros((self.n_trials, len(lams)))
            if mean
            else {
                key: np.zeros((self.n_trials, len(lams)))
    ...
    def _score(self, X, mXs, mean=True):
        ...
        PDY = {
            key: np.dot(self.P[key], np.dot(self.D[key].T, X))
    ...
    def _add_regularization(self, Y, mYs, lam, SVD=None, pre_reg=False):
        ...
        if SVD is not None:
            ...
            pregY = np.dot(
                ...
                np.linalg.inv(
                    np.dot(Y.reshape((n_features, -1)), Y.reshape((n_features, -1)).T)
    ...
    def _fit(self, X, trialX=None, mXs=None, center=True, SVD=None, optimize=True):
        ...
        if self.opt_regularizer_flag and optimize:
            ...
            if trialX is None:
                raise ValueError(
                    "To optimize the regularization parameter, the trial-by-trial data trialX needs to be provided."
                )
    ...
    def _get_n_samples(self, trialX, protect=None):
        ...
        n_unprotect = (
            len(trialX.shape) - len(protect) - 1
            if protect is not None
            else len(trialX.shape) - 1
        )
        n_protect = len(protect) if protect is not None else 0

        return trialX.shape[0] - np.sum(
            np.isnan(
                trialX[(np.s_[:],) + (np.s_[:],) * n_unprotect + (0,) * n_protect]
            ),
            0,
        )
    ...
    def _check_protected(self, X, protect):
        ...
        if protect is None:
            ...
            if set(protect) == set(
                np.arange(len(self.labels) - len(protect), len(self.labels))
            ):
                protected = True
            else:
                protected = False
    ...
    def train_test_split(self, X, trialX, N_samples=None, sample_ax=0):
        ...
        n_unprotect = (
            len(X.shape) - len(protect) if protect is not None else len(X.shape)
        )
        n_protect = len(protect) if protect is not None else 0
        ...
        trainX = (
            ...
            * (N_samples / (N_samples - 1))[
                (np.s_[:],) * n_unprotect + (None,) * n_protect
            ]
            - blindX / (N_samples - 1)[(np.s_[:],) * n_unprotect + (None,) * n_protect]
        )
    ...
    def significance_analysis(
        ...
        def compute_mean_score(X, trialX, n_splits):
            ...
            if type(self.n_components) == int:
                scores = {
                    key: np.empty((self.n_components, n_splits, K)) for key in keys
                }
            else:
                scores = {
                    key: np.empty((self.n_components[key], n_splits, K)) for key in keys
                }
            ...
            for shuffle in range(n_splits):
                ...
                for key in keys:
                    ...
                    for comp in range(ncomps):
                        scores[key][comp, shuffle] = classification(
                            trainZ[key][comp], validZ[key][comp]
                        )
        ...
        for it in range(n_shuffles):
            print(
                "\rCompute score of shuffled data: ",
                str(it),
                "/",
                str(n_shuffles),
                end=" ",
        ...
        if n_consecutive > 1:
            for key in keys:
                ...
                for k in range(mask.shape[0]):
                    masks[key][k, :] = denoise_mask(
                        masks[key][k].astype(np.int32), n_consecutive
                    )
    ...
    def transform(self, X, marginalization=None):
        ...
        def marginal_variances(marginal):
            ...
            return [
                np.sum(np.dot(D[:, k], Xr) ** 2) / total_variance
