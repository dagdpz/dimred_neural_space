...
def main(
    ...
    # ------------------------------------------------------------
    # Interaction labels for decoding
    # ------------------------------------------------------------
    ...
    df["effector_x_hand"] = df["E_code"] * df["H_code"]
    df["effector_x_space"] = df["E_code"] * df["T_code"]

    df["effector_x_hand"] = df["effector_x_hand"].map(
        {
            1: "same_EH",
            -1: "opposite_EH",
        }
    ...
    df["effector_x_space"] = df["effector_x_space"].map(
        {
            1: "same_ET",
            -1: "opposite_ET",
        }
    ...
    # ------------------------------------------------------------
    dec_effector, info_effector = time_resolved_logistic_decoding(
        df,
        label_col="effector",
        unit_col="unit_ID",
    ...
    dec_effector.to_csv(
        plots_dir / "decoding_effector_accuracy.csv",
        index=False,
    )
    ...
    # ------------------------------------------------------------
    dec_space, info_space = time_resolved_logistic_decoding(
        df,
    ...
    dec_space.to_csv(
        plots_dir / "decoding_space_accuracy.csv",
        index=False,
    )
    ...
    dec_hand, info_hand = time_resolved_logistic_decoding(
        df,
    ...
    dec_hand.to_csv(
        plots_dir / "decoding_reach_hand_accuracy.csv",
        index=False,
    )

    # ------------------------------------------------------------
    # Time-resolved decoding: effector x hand interaction
    # ------------------------------------------------------------
    dec_effector_x_hand, info_effector_x_hand = time_resolved_logistic_decoding(
        df,
    ...
    dec_effector_x_hand.to_csv(
        plots_dir / "decoding_effector_x_hand_accuracy.csv",
        index=False,
    )

    # ------------------------------------------------------------
    # Time-resolved decoding: effector x space interaction
    # ------------------------------------------------------------
    dec_effector_x_space, info_effector_x_space = time_resolved_logistic_decoding(
        df,
    ...
    dec_effector_x_space.to_csv(
        plots_dir / "decoding_effector_x_space_accuracy.csv",
        index=False,
    )
    ...
    # ------------------------------------------------------------
    plot_multiple_time_resolved_decoding(
        {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--plots_dir",
        type=Path,
        default=Path("plots/time_resolved_decoding"),
