...
def process_labels_and_filter(df):
    ...
    df["reach_hand"] = [
        ipsi_contra(side, ref)
        for side, ref in zip(reach_side, df["pulvinar_hemifield"])
    ]
    ...
    df["target_hemifield"] = [
        ipsi_contra(side, ref)
        for side, ref in zip(target_side, df["pulvinar_hemifield"])
    ]
...
def remove_invalid_rate_rows(
    ...
    *,
    ...
    valid = df[rate_col].apply(_valid_array)

    if time_col in df.columns:
        valid = valid & df[time_col].apply(_valid_array)
